define(['./module'], function (controllers) {
  ...
  controllers.controller('StatisticsCtrl',
      ['$scope',
        '$state',
        '$rootScope',
        'configService',
        ...
        function ($scope, $state, $rootScope, configService, dashboard, statCalculations) {
          ...
          function _getData() {
            ...
            dashboard.findResDailyStatistics(start,end).then((results) => {
              ...
              let dar = [];
              let o = $scope.mData.map((m) => {return m.occupancy});
              let a = $scope.mData.map((m) => {return m.ADR});
              let r = $scope.mData.map((m) => {return m.RevPAR});
              dar.push(o);
              dar.push(a);
              dar.push(r);
              $scope.data = dar;
