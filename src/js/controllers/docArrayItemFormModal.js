...
define(['./module'], function (controllers) {
  ...
  controllers.controller('DocArrayItemFormModalCtrl', ['$scope',
    '$modalInstance',
    'modalParams',
    'Itemtype',
    'AppConstants',
    ...
    function ($scope, $modalInstance, modalParams, ItemType, AppConstants, configService, utility, dbEnums) {
      ...
      AppConstants.find(function (err, constants) {
        ...
        switch (mode) {
          ...
          case 'r':
            ...
            _findItemInDoc(modalParams.data).then((itemType) => {
                if (itemType) {
      ...
      $scope.delete = function (err) {
        let icnt = modalParams.docArray.length;
        ...
        if (modalParams.docArray.length === icnt) {
          ...
          helpers.autoClose(msg, $scope.itemType._id);
        }
      ...
      // Mongoose FindOne (promise) method.
      function _findItemInDoc(id) {
        return new Promise((resolve, reject) => {
          let isNum = typeof (id) === 'number' || (!isNaN(parseFloat(id)) && isFinite(id));
          let found = false;
          let item = null;
          let ix;
